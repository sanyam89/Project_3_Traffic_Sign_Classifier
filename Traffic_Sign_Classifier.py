...
def LeNet(x):    
    ...
    fw1 = tf.Variable(tf.truncated_normal((5, 5, 3, 6), mean = mu, stddev = sigma)) # (height, width, input_depth, output_depth)
    ...
    fw2 = tf.Variable(tf.truncated_normal((5, 5, 6, 16), mean = mu, stddev = sigma)) # (height, width, input_depth, output_depth)
...
new_images = 
new_images_classification = [1,2,6,8,0]
with tf.Session() as sess:
    saver.restore(sess, tf.train.latest_checkpoint('.'))
